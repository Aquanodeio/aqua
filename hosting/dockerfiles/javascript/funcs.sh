...
js_install_and_build() {
 ...
 # Installing dependencies
  ...
  if [ "$FRAMEWORK" = "HTML" ] || [ "$FRAMEWORK" = "Nuxt.js" ] || [ "$FRAMEWORK" = "Vitepress" ]; then
    :
  elif [ "$FRAMEWORK" = "Express.js" ]; then
    echo "[*] Skipping build for Express.js..."
...
# Function to execute run commands
execute_run_commands() {
    if [ -n "$RUN_COMMANDS" ]; then
        echo "[*] Executing run commands..."
        ...
        echo "[*] All run commands completed successfully"
        return 0
    fi
}
...
js_serve_app() {
  ...
  if [ "$BUILD_DIRECTORY" ]; then
  ...
  elif [ "$FRAMEWORK" = "Vue.js" ] || [ "$FRAMEWORK" = "Gridsome" ] || [ "$FRAMEWORK" = "Umi.js" ] || [ "$FRAMEWORK" = "Astro.js" ] || [ "$FRAMEWORK" = "Vite-react.js" ] || [ "$FRAMEWORK" = "Ember.js" ]; then
    ...
    if npm start > /dev/null 2>&1; then
      ...
      echo "[*] Error: No entry point (index.js/app.js/server.js) found for Express.js"
      exit 1
    fi
